...
router = APIRouter(prefix="/form_types", tags=["FormTypes"])
...
def find_all(
    ...
    return get_form_type_service(db_session).read_all_form_types(request, is_include_extras)


@router.get("/{form_type_id}", response_model=FormTypeResponse, status_code=HTTPStatus.OK)
def find_one(
    form_type_id: int,
    request: Request,
    is_include_extras: bool = False,
    ...
    validate_http_basic_credentials(request, http_basic_credentials)
    form_type_response: FormTypeResponse = get_form_type_service(db_session).read_one_form_type(
        form_type_id, request, is_include_extras
    )
...
def insert_one(
    ...
    validate_http_basic_credentials(request, http_basic_credentials)
    return get_form_type_service(db_session).create_one_form_type(request, form_type_request)


@router.delete("/{form_type_id}", response_model=FormTypeResponse, status_code=HTTPStatus.OK)
def delete_one(
    form_type_id: int,
    request: Request,
    http_basic_credentials: HTTPBasicCredentials = Depends(http_basic_security),
...
@router.put("/{form_type_id}", response_model=FormTypeResponse, status_code=HTTPStatus.OK)
def update_one(
    form_type_id: int,
    request: Request,
    form_type_request: FormTypeRequest,
