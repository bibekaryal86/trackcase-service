...
class CrudActions:
    ...
    def get_by_id(self, model_id: int) -> BaseSchema:
        return self.db_session.query(self.db_model).filter(self.db_model.id == model_id).first()

    def get_all(self, skip: int = 0, limit: int = 1000) -> List[BaseSchema]:
        records = self.db_session.query(self.db_model).offset(skip).limit(limit).all()
        return records
